...
function inning(){
    const score1 = Math.round(Math.random() * 2);
    return score1;
}
...
function finalScore(callback,numOfInns){
  let Home = 0;
  let Away = 0;
...
function scoreboard(innScoreCB,inningCB,numOfInns) {
  const scoreByInning = [];
  let homeScore = 0;
  let awayScore = 0;
  for(let i = 0; i < numOfInns; i++){
    const currentInning = innScoreCB(inningCB);
